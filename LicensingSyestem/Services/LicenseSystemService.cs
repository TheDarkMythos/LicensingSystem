...
namespace LicensingSyestem.Services
{
    public class LicenseSystemService : ILicenseSystemService
    {
        ...
        public async Task<int> AddClientAsync(string lisenseCode, Client client)
        {
            ...
            var license = await dbContext.Licenses.FirstOrDefaultAsync(q => q.Code == lisenseCode);
        ...
        {
            var IsUserLimited = await dbContext.Clients.CountAsync(q => q.LicenseId.Equals(licenseid)) >= MaxCount;
            return IsUserLimited;
        }

        public async Task<bool> IsUserLimitReached(string lisenseCode)
        {
            var license = await dbContext.Licenses.FirstOrDefaultAsync(q => q.Code == lisenseCode);
        ...
        {
            var license = await dbContext.Licenses.FirstOrDefaultAsync(q => q.Id == Id);
            if (license == null || !license.Active || (license.DeactiveateAfterExpiration && license.ExpirationDate.HasValue && license.ExpirationDate.Value > DateTime.Now))
                return 0;
        ...
        {
            var lic = await dbContext.Licenses.FirstOrDefaultAsync(w => w.Code == licenseCode);
            if (lic == null || !lic.Active) return false;

            var clinet = await dbContext.Clients.FirstOrDefaultAsync(q => q.LicenseId == lic.Id && q.UniqueID == UniqueId && q.DeviceID == DeviceId);
